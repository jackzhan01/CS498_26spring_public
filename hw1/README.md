...
1. **Familiarize yourself with CloudLab and Preparation**
   ...
   * We recommend using VS Code Remote SSH to write and test your code on the cluster. 1. Open a new VS Code window. 2. Press Ctrl + Alt + P, type Remote-SSH, and select Remote-SSH: Connect Current Window to Host... 3. Under **`List View`** in my experiment on Cloudlab , copy and paste the provided SSH command to connect. 4. Repeat the steps above three times (open three VS Code windows) to connect to all three nodes. 5. Once connected, run git clone on each node to download the lab assignment repository.
   * **(Important!)Write your netid in netid.txt.**
...
4. **Unit test in `run.py`.** A basic test at the end of `run.py` checks whether the final model parameters are identical across the three nodes. We strongly recommend that you **pass this unit test before submitting** your homework; otherwise, you may receive a **significant** deduction in marks.
